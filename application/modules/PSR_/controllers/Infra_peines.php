...
class Infra_peines extends CI_Controller
{
 ...
 function index()
 {
   $data['title'] = 'La liste des infranctions';
 ...
 {
  $i=1;
  $query_principal='SELECT ID_INFRA_PEINE, ii.NIVEAU_ALERTE as NIVEAU_ALERTE, AMENDES, MONTANT, POINTS FROM infra_peines ip LEFT JOIN infra_infractions ii on ii.ID_INFRA_INFRACTION=ip.ID_INFRA_INFRACTION WHERE 1';
  ...
  foreach ($fetch_psr as $row)
  {
    ...
    <a class='btn btn-danger btn-md' href='" . base_url('ihm/psr_elements/delete/'.$row->ID_INFRA_PEINE) . "'>Supprimer</a>
