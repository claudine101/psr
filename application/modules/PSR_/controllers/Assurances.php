...
class Assurances extends CI_Controller
{
	...
	{
		$i=1;
		$query_principal="SELECT ID_ASSURANCE, NUMERO_PLAQUE, assurances_vehicules.ID_ASSUREUR,assureur.ASSURANCE, DATE_DEBUT, DATE_VALIDITE, PLACES_ASSURES, TYPE_ASSURANCE, NOM_PROPRIETAIRE FROM assurances_vehicules left join assureur on assureur.ID_ASSUREUR=assurances_vehicules.ID_ASSUREUR WHERE 1";
		...
		$order_column=array('NUMERO_PLAQUE','NOM_ASSUREUR','NOM_PROPRIETAIRE');

		$order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : ' ORDER BY Nom ASC';

		$search = !empty($_POST['search']['value']) ? ("AND NUMERO_PLAQUE LIKE '%$var_search%' OR NOM_ASSUREUR LIKE '%$var_search%' OR NOM_PROPRIETAIRE LIKE '%$var_search%'  "):'';     
