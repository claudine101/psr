...
class Droit extends CI_Controller
{
	...
	function listing()
	{
		...
		$order_by='';
		$order_column=array('STATUS','INFRACTION','PSR_ELEMENT','PEINES','AFFECTATION','PERMIS');

		$order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : ' ORDER BY PEINES,PERMIS ASC';

		$search = !empty($_POST['search']['value']) ? ("AND PEINES LIKE '%$var_search%' OR AFFECTATION LIKE '%$var_search%' OR PERMIS LIKE '%$var_search%'  "):'';     
