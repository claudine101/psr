...
class Signalement_accident extends CI_Controller
{
	...
	function index()
	{
		$accident = $this->Modele->getRequete('SELECT date_format(DATE_INSERTION,"%d-%m-%Y") as date FROM sign_accident WHERE 1 GROUP BY date_format(DATE_INSERTION,"%d-%m-%Y")');
	...
	function listing()
	{
        ...
        $critaire_date=" AND  date_format(sa.DATE_INSERTION,'%d-%m-%Y') BETWEEN '".$date1."' AND '".$date2."' ";

        }
        if(!empty($date1) && empty($date2)){

              $critaire_date=" AND  date_format(sa.DATE_INSERTION,'%d-%m-%Y') = '".$date1."'  ";

          }

        if(!empty($date2) && empty($date1)){

            $critaire_date=" AND  date_format(sa.DATE_INSERTION,'%d-%m-%Y') = '".$date2."'  ";

         }


		$query_principal = 'SELECT ID_SIGN_ACCIDENT,concat(us.NOM_CITOYEN, " ", us.PRENOM_CITOYEN) as name,chaussee.NOM_CHAUSSE,us.NOM_UTILISATEUR, PLAQUE, ID_GRAVITE ,tg.DESCRIPTION AS GRAVITE , USER_ID,sa.DESCRIPTION, IMAGE_1, IMAGE_2, IMAGE_3,sa.ID_CHAUSSEE, VALIDATION,sa.LONGITUDE, sa.LATITUDE, sa.DATE_INSERTION,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,sa.STATUT_TRAITEMENT,ci.ID_ALERT_STATUT, ci.NOM FROM sign_accident sa LEFT JOIN utilisateurs us ON us.ID_UTILISATEUR=sa.USER_ID LEFT JOIN type_gravite tg ON tg.ID_TYPE_GRAVITE=sa.ID_GRAVITE LEFT JOIN chaussee ON chaussee.ID_CHAUSSEE=sa.ID_CHAUSSEE  LEFT JOIN statut_validation sv on sv.ID_STATUT=sa.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts ci ON sa.ID_ALERT_STATUT = ci.ID_ALERT_STATUT WHERE 1' . $condition1.' '.$critaire_date;
		...
		foreach ($fetch_psr as $row) {
            ...
            $option = "<a class='btn btn-primary' href='javascript:;'' onclick='getDetailControl(".$row->ID_SIGN_ACCIDENT.")'' >Traiter</i></a>";
             if ($row->ID_ALERT_STATUT==4) {
					$option = "<a class='btn btn-primary' href='javascript:;'' onclick='fin_trait(".$row->ID_SIGN_ACCIDENT.")'' >Marqué comme terminer</i></a>";
	...
	function delete()
	{
		...
		$data['alerte']=$this->Model->getRequeteOne('SELECT ID_SIGN_ACCIDENT,concat(us.NOM_CITOYEN, " ", us.PRENOM_CITOYEN) as name,chaussee.NOM_CHAUSSE,us.NOM_UTILISATEUR, PLAQUE, ID_GRAVITE ,tg.DESCRIPTION AS GRAVITE , USER_ID,sa.DESCRIPTION, IMAGE_1, IMAGE_2, IMAGE_3,sa.ID_CHAUSSEE, VALIDATION,sa.LONGITUDE, sa.LATITUDE, sa.DATE_INSERTION date,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,sa.STATUT_TRAITEMENT, ci.NOM FROM sign_accident sa LEFT JOIN utilisateurs us ON us.ID_UTILISATEUR=sa.USER_ID LEFT JOIN type_gravite tg ON tg.ID_TYPE_GRAVITE=sa.ID_GRAVITE LEFT JOIN chaussee ON chaussee.ID_CHAUSSEE=sa.ID_CHAUSSEE  LEFT JOIN statut_validation sv on sv.ID_STATUT=sa.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts ci ON sa.ID_ALERT_STATUT = ci.ID_ALERT_STATUT WHERE ID_SIGN_ACCIDENT ='.$id.'');

		$titre = "Detail d'une alerte";

		$map = $this->load->view('signaler/signaler_accident_det_v', $data, TRUE);
	...
	function traiter_conf($id){
         $data = $this->Model->update('sign_accident',array('ID_SIGN_ACCIDENT'=>$id),array('ID_ALERT_STATUT'=>4));
            if($data){
	...
	function traiter_rejet($id){
         $data = $this->Model->update('sign_accident',array('ID_SIGN_ACCIDENT'=>$id),array('ID_ALERT_STATUT'=>5));
            if($data){
            $output = array('status'=> true,'id'=>$id);	
            }
	...
	function fin_valide($id){
		$data = $this->Model->update('sign_accident',array('ID_SIGN_ACCIDENT'=>$id),array('ID_ALERT_STATUT'=>6));
        if($data){
            $output = array('status'=> true,'id'=>$id);	
        }
