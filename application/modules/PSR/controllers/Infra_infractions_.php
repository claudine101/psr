...
class  Infra_infractions extends CI_Controller
{
	...
	function listing()
	{
		...
		$query_principal='SELECT ID_INFRA_INFRACTION, NIVEAU_ALERTE FROM infra_infractions WHERE 1';
		...
		$order_column=array('ID_INFRA_INFRACTION','NIVEAU_ALERTE');
		...
		foreach ($fetch_infraction as $row)
		{
			...
			<center><h5><strong>Voulez-vous supprimer?</strong> <br><b style='background-color:prink;color:green;'><i>" .$row->NIVEAU_ALERTE." </i></b></h5></center>
			</div>
	...
	function getOne()
	{
		...
		$data['data']=$this->Modele->getOne('infra_infractions
			',array('ID_INFRA_INFRACTION'=>$id));
	...
	function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Modele->update('infra_infractions',array('ID_INFRA_INFRACTION'=>$id),$data);
