...
class Vol_Controller extends CI_Controller
{
	...
	function index()
	{
		$vol = $this->Modele->getRequete('SELECT date_format(DATE_INSERTION,"%d-%m-%Y") as date FROM sign_tampo_pj WHERE 1 GROUP BY date_format(DATE_INSERTION,"%d-%m-%Y")');
		$data['vol'] = $vol;
	...
	function listing()
	{
        ...
        $critaire_date=" AND  date_format(pj.DATE_INSERTION,'%d-%m-%Y') BETWEEN '".$date1."' AND '".$date2."' ";

        }
        if(!empty($date1) && empty($date2)){

              $critaire_date=" AND  date_format(pj.DATE_INSERTION,'%d-%m-%Y') = '".$date1."'  ";

          }

        if(!empty($date2) && empty($date1)){

            $critaire_date=" AND  date_format(pj.DATE_INSERTION,'%d-%m-%Y') = '".$date2."'  ";

         }

		$query_principal = 'SELECT ID_TAMPO_PJ,pj.NUMERO_SERIE,type_couleur.COULEUR,obr.ID_IMMATRICULATION,obr.NUMERO_PLAQUE,IMAGE_1, IMAGE_2,pj.MARQUE_VOITURE,DATE_VOLER,STATUT,VALIDATION,pj.DATE_INSERTION,us.NOM_UTILISATEUR, pj.NUMERO_PERMIS,c.NOM_PROPRIETAIRE,c.ID_PERMIS, pj.DESCRIPTION ,sv.DESCRIPTION as STATUT_TRAITEM,sv.ID_STATUT as status,pj.ID_ALERT_STATUT,hvv.ID_SIGNALEMET,pj.STATUT_TRAITEMENT, civil_alerts_statuts.NOM FROM sign_tampo_pj pj LEFT JOIN utilisateurs us on us.ID_UTILISATEUR=pj.USER_ID LEFT JOIN obr_immatriculations_voitures obr ON obr.ID_IMMATRICULATION=pj.ID_OBR LEFT JOIN type_couleur on type_couleur.ID_TYPE_COULEUR =pj.ID_COULEUR LEFT JOIN chauffeur_permis c ON TRIM(c.NUMERO_PERMIS)=TRIM(pj.NUMERO_PERMIS) LEFT JOIN statut_validation sv on sv.ID_STATUT=pj.STATUT_TRAITEMENT LEFT JOIN histo_validation_vol hvv ON hvv.ID_SIGNALEMET=pj.ID_TAMPO_PJ LEFT JOIN civil_alerts_statuts on pj.ID_ALERT_STATUT=civil_alerts_statuts.ID_ALERT_STATUT WHERE 1 ' . $condition1 . ' '.$critaire_date.' GROUP BY ID_TAMPO_PJ ,c.NOM_PROPRIETAIRE,c.ID_PERMIS';
		...
		foreach ($fetch_psr as $row) {
			...
			$option = "<a class='btn btn-primary' href='javascript:;'' onclick='getDetailControl(".$row->ID_TAMPO_PJ.")'' >Traiter</i></a>";
				if ($row->ID_ALERT_STATUT==4) {
					$option = "<a class='btn btn-primary' href='javascript:;'' onclick='fin_trait(".$row->ID_TAMPO_PJ.")'' >Marqué comme terminer</i></a>";
	...
	function delete()
	{
	 ...
	 function detail($id){
		...
		$data['alerte']=$this->Model->getRequeteOne('SELECT ID_TAMPO_PJ,pj.NUMERO_SERIE,type_couleur.COULEUR,obr.ID_IMMATRICULATION,obr.NUMERO_PLAQUE,IMAGE_1, IMAGE_2,pj.MARQUE_VOITURE,DATE_VOLER as dat,STATUT,VALIDATION,pj.DATE_INSERTION as date ,us.NOM_UTILISATEUR, pj.NUMERO_PERMIS,c.NOM_PROPRIETAIRE,c.ID_PERMIS, pj.DESCRIPTION ,sv.DESCRIPTION as STATUT_TRAITEM,sv.ID_STATUT as status,hvv.ID_SIGNALEMET,pj.STATUT_TRAITEMENT, civil_alerts_statuts.NOM FROM sign_tampo_pj pj LEFT JOIN utilisateurs us on us.ID_UTILISATEUR=pj.USER_ID LEFT JOIN obr_immatriculations_voitures obr ON obr.ID_IMMATRICULATION=pj.ID_OBR LEFT JOIN type_couleur on type_couleur.ID_TYPE_COULEUR =pj.ID_COULEUR LEFT JOIN chauffeur_permis c ON TRIM(c.NUMERO_PERMIS)=TRIM(pj.NUMERO_PERMIS) LEFT JOIN statut_validation sv on sv.ID_STATUT=pj.STATUT_TRAITEMENT LEFT JOIN histo_validation_vol hvv ON hvv.ID_SIGNALEMET=pj.ID_TAMPO_PJ LEFT JOIN civil_alerts_statuts on pj.ID_ALERT_STATUT=civil_alerts_statuts.ID_ALERT_STATUT WHERE ID_TAMPO_PJ ='.$id.'');

		$titre = "Detail d'une alerte";

		$map = $this->load->view('vol_det_v', $data, TRUE);
	...
	function traiter_conf($id){
         $data = $this->Model->update('sign_tampo_pj',array('ID_TAMPO_PJ'=>$id),array('ID_ALERT_STATUT'=>4));
            if($data){
	...
	function traiter_rejet($id){
         $data = $this->Model->update('sign_tampo_pj',array('ID_TAMPO_PJ'=>$id),array('ID_ALERT_STATUT'=>5));
            if($data){
            $output = array('status'=> true,'id'=>$id);	
            }
	...
	function fin_valide($id){
		$data = $this->Model->update('sign_tampo_pj',array('ID_TAMPO_PJ'=>$id),array('ID_ALERT_STATUT'=>6));
        if($data){
            $output = array('status'=> true,'id'=>$id);	
        }
