...
class Validation_signalement extends CI_Controller
{
  ...
  $this->sendNotifactions_rejetter($id);
        ...
        $profil = $this->Modele->getRequeteOne('SELECT pro.PROFIL_ID FROM profil pro LEFT JOIN utilisateurs u ON u.PROFIL_ID=pro.PROFIL_ID   WHERE u.ID_UTILISATEUR='.$id_utilisateur);
          ...
          else if($profil['PROFIL_ID']==19)
         {
            $data = array('ID_SIGNALEMENT_NEW' =>$id ,
               ...
               $this->sendNotifactions_valider($id);
         }
...
function sendNotifactions_valider($ID_SIGNALEMENT_NEW){

   $tokens = $this->Model->getRequete('SELECT scan.PLAQUE_NUMERO,scan.CODE_SUIVIS,u.ID_UTILISATEUR,n.TOKEN ,u.PRENOM_CITOYEN,u.NOM_CITOYEN,u.NUMERO_CITOYEN FROM notification_tokens n JOIN  signalement_civil_agent_new scan ON scan.ID_UTILISATEUR=n.ID_UTILISATEUR LEFT JOIN utilisateurs u ON u.ID_UTILISATEUR=scan.ID_UTILISATEUR WHERE scan.ID_SIGNALEMENT_NEW = '.$ID_SIGNALEMENT_NEW);
    if (!empty($tokens)) {
                ...
                foreach ($tokens as $key => $value) {
                  ...
                  $titre =  "Votre signalement (" .$value['CODE_SUIVIS'].") a été approuvé ";
                  $messageVehicule = "Nous avons pris en compte votre signalement et des mesures seront prises. Merci de nous aider à améliorer la sécurité routière ";
                  $donnes = array(
                   'TITRE'=>$titre,
                  ...
                  $data = array(
                    ...
                    'NUMERO_PLAQUE'=>$value['PLAQUE_NUMERO']?$value['PLAQUE_NUMERO']:null,
                    'IS_AGENT_PSR'=>1,
                    'STATUT'=>1,
...
function sendNotifactions_rejetter($ID_SIGNALEMENT_NEW){
   ...
   $tokens = $this->Model->getRequete('SELECT scan.PLAQUE_NUMERO,scan.CODE_SUIVIS,u.ID_UTILISATEUR,n.TOKEN ,u.PRENOM_CITOYEN,u.NOM_CITOYEN,u.NUMERO_CITOYEN FROM notification_tokens n JOIN  signalement_civil_agent_new scan ON scan.ID_UTILISATEUR=n.ID_UTILISATEUR LEFT JOIN utilisateurs u ON u.ID_UTILISATEUR=scan.ID_UTILISATEUR WHERE scan.ID_SIGNALEMENT_NEW = '.$ID_SIGNALEMENT_NEW);
    if (!empty($tokens)) {
                ...
                foreach ($tokens as $key => $value) {
                  ...
                  $titre =  "Désolé, votre signalement (" .$value['CODE_SUIVIS'].") n'a pas pu être validé Votre signalement ";
                  $message = "Nous avons examiné votre signalement, mais nous n'avons pas été en mesure de le valider. Nous vous remercions de votre contribution et nous encourageons à signaler toute autre infraction que vous pouvez observer à l'avenir. ";
                  $donnes = array(
                   'TITRE'=>$titre,
                   'CONTENU'=>$message,
                   'tokens'=> $tokesss
