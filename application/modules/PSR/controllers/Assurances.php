...
class Assurances extends CI_Controller
{
	...
	function index()
	{
		...
		$data['assurances'] = $this->Model->getRequete('SELECT ID_ASSUREUR, ASSURANCE FROM assureur WHERE 1  ORDER BY ASSURANCE ASC');
	...
	function listing()
	{
	   ...
	   $inputs = !empty($this->input->post('ID_ASSUREUR')) ? $this->input->post('ID_ASSUREUR') : "";

	   $id_assureur =  !empty($inputs) ? "  AND assurances_vehicules.ID_ASSUREUR =".$inputs." " : " ";

	   $id_assureur = !empty($this->input->post('ID_ASSUREUR0')) ? "  AND assurances_vehicules.ID_ASSUREUR =".$this->input->post('ID_ASSUREUR0')." " : $id_assureur;
		...
		$order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY DATE_INSERTION ';

		$search = !empty($_POST['search']['value']) ? ("AND assurances_vehicules.NUMERO_PLAQUE LIKE '%$var_search%' OR ASSURANCE LIKE '%$var_search%' OR assurances_vehicules.NOM_PROPRIETAIRE LIKE '%$var_search%' OR assurances_vehicules.DATE_INSERTION LIKE '%$var_search%'  ") : '';
