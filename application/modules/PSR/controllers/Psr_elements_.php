...
class Psr_elements extends CI_Controller
{
	...
	function listing()
	{
		...
		$i=1;
		$query_principal='SELECT ID_PSR_ELEMENT, NOM, PRENOM, NUMERO_MATRICULE,sp.PROVINCE_NAME AS province, sc.COMMUNE_NAME AS commune ,sz.ZONE_NAME AS zone, sco.COLLINE_NAME AS Colline, TELEPHONE, EMAIL,PHOTO, psr.LATITUDE, psr.LONGITUDE, psr.IS_ACTIF, ID_GRADE, p.STATUT FROM psr_elements psr LEFT JOIN syst_provinces sp ON psr.PROVINCE_ID=sp.PROVINCE_ID LEFT JOIN syst_communes sc ON sc.COMMUNE_ID=psr.COMMUNE_ID LEFT JOIN syst_zones sz on sz.ZONE_ID=psr.ZONE_ID LEFT JOIN syst_collines sco ON sco.COLLINE_ID=psr.COLLINE_ID LEFT JOIN utilisateurs us on us.PSR_ELEMENT_ID=psr.ID_PSR_ELEMENT LEFT JOIN profil p ON p.PROFIL_ID = us.PROFIL_ID  WHERE 1';
		...
		foreach ($fetch_psr as $row)
		{
			...
			$sub_array = array();
			$sub_array[]=$i++;
			$sub_array[]='<a href="'.$row->PHOTO.'" target="_blank" ><img style="border-radius:50%;width:40px;height:40px" src="'.$row->PHOTO.'"></a>';
			$sub_array[]=$row->NOM;
