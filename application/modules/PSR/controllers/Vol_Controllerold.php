...
class Vol_Controller extends CI_Controller
{
	...
	function listing()
	{
		...
		if ($this->session->userdata('PROFIL_ID') == 1) {
			$condition1 = " AND pj.STATUT_TRAITEMENT=2 or pj.STATUT_TRAITEMENT=4";
		...
		foreach ($fetch_psr as $row) {
			...
			$name = '<table> <tbody><tr><td><img alt="Avtar" style="border-radius:50%;width:30px;height:30px" src="https://app.mediabox.bi/wasiliEate/uploads/personne.png"></td><td>' . $row->NOM_PROPRIETAIRE . ' <font style="font-size:10px;color:red"><i class="far fa-comment-dots"></i></font></td></tr></tbody></table>';
