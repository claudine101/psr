...
class Signalement_embouteillage extends CI_Controller
{
	...
	function index()
	{
		$embouteillage = $this->Modele->getRequete('SELECT date_format(DATE_INSERTION,"%d-%m-%Y") as date FROM sign_ambouteillage WHERE 1 GROUP BY date_format(DATE_INSERTION,"%d-%m-%Y")');
		$data['embouteillage'] = $embouteillage;
	...
	function listing()
	{
		...
		$query_principal = 'SELECT ID_AMBOUTEILLAGE, sa.ID_CHAUSSEE ,chaussee.NOM_CHAUSSE,us.NOM_UTILISATEUR, CAUSE,concat(us.NOM_CITOYEN, " ", us.PRENOM_CITOYEN) as name, IMAGE_1, IMAGE_2, IMAGE_3, VALIDATION, sa.LONGITUDE, sa.LATITUDE, USER_ID, sa.DATE_INSERTION,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,sa.STATUT_TRAITEMENT, civil_alerts_statuts.ID_ALERT_STATUT,civil_alerts_statuts.NOM  FROM sign_ambouteillage sa LEFT JOIN utilisateurs us ON us.ID_UTILISATEUR=sa.USER_ID LEFT JOIN chaussee ON chaussee.ID_CHAUSSEE=sa.ID_CHAUSSEE  LEFT JOIN statut_validation sv on sv.ID_STATUT=sa.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts on sa.ID_ALERT_STATUT=civil_alerts_statuts.ID_ALERT_STATUT WHERE 1' . $condition1.' '.$critaire_date;
    ...
    // echo $query_principal ;
        ...
        if (!empty($order_by)){
           ...
           $order_by = isset($_POST['order']) ? ' ORDER BY '.$_POST['order']['0']['column'] .'  '.$_POST['order']['0']['dir'] : ' ORDER BY sa.DATE_INSERTION DESC';
        }
		...
		foreach ($fetch_psr as $row)
		{
			...
			$option = "<a class='btn btn-primary' href='javascript:;'' onclick='getDetailControl(".$row->ID_AMBOUTEILLAGE.")'' >Traiter</i></a>";
			if ($row->ID_ALERT_STATUT==4) {
					$option = "<a class='btn btn-primary' href='javascript:;'' onclick='fin_trait(".$row->ID_AMBOUTEILLAGE.")'' >Marqué comme terminer</i></a>";
	...
	function delete()
	{
	 ...
	 function detail($id){
		...
		$data['alerte']=$this->Model->getRequeteOne('SELECT ID_AMBOUTEILLAGE, sa.ID_CHAUSSEE ,chaussee.NOM_CHAUSSE,us.NOM_UTILISATEUR, CAUSE,concat(us.NOM_CITOYEN, " ", us.PRENOM_CITOYEN) as name, IMAGE_1, IMAGE_2, IMAGE_3, VALIDATION, sa.LONGITUDE, sa.LATITUDE, USER_ID, sa.DATE_INSERTION as date ,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,sa.STATUT_TRAITEMENT,civil_alerts_statuts.NOM  FROM sign_ambouteillage sa LEFT JOIN utilisateurs us ON us.ID_UTILISATEUR=sa.USER_ID LEFT JOIN chaussee ON chaussee.ID_CHAUSSEE=sa.ID_CHAUSSEE  LEFT JOIN statut_validation sv on sv.ID_STATUT=sa.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts on sa.ID_ALERT_STATUT=civil_alerts_statuts.ID_ALERT_STATUT WHERE ID_AMBOUTEILLAGE='.$id.'');

		$titre = "Detail d'une alerte";

		$map = $this->load->view('signaler/signaler_embouch_det_v', $data, TRUE);
	...
	function traiter_conf($id){
         $data = $this->Model->update('sign_ambouteillage',array('ID_AMBOUTEILLAGE'=>$id),array('ID_ALERT_STATUT'=>4));
            if($data){
	...
	function traiter_rejet($id){
         $data = $this->Model->update('sign_ambouteillage',array('ID_AMBOUTEILLAGE'=>$id),array('ID_ALERT_STATUT'=>5));
            if($data){
            $output = array('status'=> true,'id'=>$id);	
            }
	...
	function fin_valide($id){
		$data = $this->Model->update('sign_ambouteillage',array('ID_AMBOUTEILLAGE'=>$id),array('ID_ALERT_STATUT'=>6));
        if($data){
            $output = array('status'=> true,'id'=>$id);	
        }
