...
//requete pour retourner les voitures dont le controle technique expire par rapport à la date actuelle 
    ...
    $requete=$this->Modele->getRequete("
      SELECT if(`DATE_VALIDITE` < CURRENT_DATE(),'invalide','valide') AS name,COUNT(DATE_VALIDITE)AS NBRE FROM `otraco_controles` WHERE 1 GROUP BY name");


    $requete2=$this->Modele->getRequete("SELECT  if(DATE_VALIDITE < CURRENT_DATE(),'invalide','valide') AS name,COUNT(`DATE_VALIDITE`)AS NBRE FROM `assurances_vehicules` WHERE 1 GROUP BY name");
