...
class Acc_Par_Chaussee extends CI_Controller
{
  ...
  function index()
  {
   ...
   // print_r($date_insert);//exit();
    ...
    //print_r($condition);
  	$i=1;
  	$Chaussee=$this->Modele->getRequete("SELECT c.ID_CHAUSSEE as ID,COUNT(acc.ID_SIGN_ACCIDENT) as NBRE,c.NOM_CHAUSSE as CHAUSSE FROM sign_accident acc left JOIN chaussee c on acc.ID_CHAUSSEE=c.ID_CHAUSSEE WHERE 1 ".$condition." GROUP BY  CHAUSSE,c.ID_CHAUSSEE");
