...
$datjour=$this->Model->getRequete('SELECT DISTINCT DATE_FORMAT(obr_immatriculations_voitures.DATE_INSERTION, "%Y-%m-%d") as mois from obr_immatriculations_voitures where DATE_FORMAT(obr_immatriculations_voitures.DATE_INSERTION, "%Y-%m")="'.$jour.'" ORDER BY mois DESC');
 ...
 $immatr=$this->Model->getRequete("SELECT DISTINCT ".$categorie." AS mois,COUNT(`ID_IMMATRICULATION`) AS NBRE FROM obr_immatriculations_voitures WHERE 1 GROUP BY mois ORDER BY mois ASC");
 ...
 series: [
{
   ...
   Highcharts.chart('container1', {
    ...
    title: {
        text: '<b>Immatriculation  par categories  </b><br>  Le ".date('d-m-Y')." '
    },
    accessibility: {
        point: {
            valueSuffix: '%'
