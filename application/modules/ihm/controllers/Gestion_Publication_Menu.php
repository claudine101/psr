 ...
 */
class Gestion_Publication_Menu extends CI_Controller
{
	...
	function index()
	{
		...
		$this->load->view('Gestion_Publication_Menu_List_View',$data);
	}
	...
	{ 
		...
		if ($ID_RESTAURANT!='') {
			# code...
			$critere=' WHERE restaurants.ID_RESTAURANT='.$ID_RESTAURANT.'';
		}

		$query_principal='SELECT ID_MENU,menu_categories.NOM_CATEGORIE,restaurants.NOM_RESTAURANT, NOM_MENU, DESCRIPTION, IMAGE_1 FROM menu LEFT JOIN menu_categories ON menu_categories.ID_CATEGORIE=menu.ID_CATEGORIE LEFT JOIN restaurants ON restaurants.ID_RESTAURANT=menu.ID_MENU '.$critere;
		...
		foreach ($fetch_menu as $row) {
			...
			$option .= "<li><a class='btn-md' href='" . base_url('ihm/Gestion_Publication_Menu/getOne/'.md5($row->ID_MENU)) . "'><label class='text-info'>&nbsp;&nbsp;Modifier</label></a></li>";
			...
			<a class='btn btn-danger btn-md' href='" . base_url('ihm/Gestion_Publication_Menu/delete/'.$row->ID_MENU) . "'>Supprimer</a>
			...
			$sub_array = array();
			$sub_array[]= '<a href="'.$row->IMAGE_1.'" target="_blank" ><img style="border-radius:50%;width:40px;height:40px" src="'.base_url('uploads/resto/'.$row->IMAGE_1).'"></a>';
			$sub_array[]='<span style="float:right;">'.$row->NOM_MENU.'</span>';
			$sub_array[]='<span style="float:right;">'.$row->NOM_CATEGORIE.'</span>';
			$sub_array[]='<span style="float:right;">'.$row->DESCRIPTION.'</span>'; 
			$sub_array[]=$option;   
			$data[] = $sub_array;
		}
	...
	function ajouter()
	{
		...
		$data['title'] = 'Nouvelle Menus';
		$this->load->view
		('Gestion_Publication_Menu_Add_View',$data);
	...
	function add()
	{
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			redirect(base_url('ihm/Gestion_Publication_Menu/'));
