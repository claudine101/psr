...
class Sign_inciden_naturel extends CI_Controller
{
	...
	function index()
	{
		$incident = $this->Modele->getRequete('SELECT date_format(DATE_INSERTION,"%d-%m-%Y") as date FROM sign_incident_naturel WHERE 1 GROUP BY date_format(DATE_INSERTION,"%d-%m-%Y")');
	...
	function listing()
	{
        ...
        $critaire_date=" AND  date_format(s.DATE_INSERTION,'%d-%m-%Y') BETWEEN '".$date1."' AND '".$date2."' ";

        }
        if(!empty($date1) && empty($date2)){

              $critaire_date=" AND  date_format(s.DATE_INSERTION,'%d-%m-%Y') <= '".$date1."'  ";

          }

        if(!empty($date2) && empty($date1)){

            $critaire_date=" AND  date_format(s.DATE_INSERTION,'%d-%m-%Y') >= '".$date2."'  ";

         }


		$query_principal = 'SELECT ID_SIGN_INCIDENT, s.DESCRIPTION, s.IMAGE_1, s.IMAGE_2, s.IMAGE_3, s.VALIDATION, s.LONGITUDE, s.LATITUDE, s.DATE_INSERTION, u.NOM_UTILISATEUR,USER_ID,concat(u.NOM_CITOYEN," ",u.PRENOM_CITOYEN)as NAME,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,s.STATUT_TRAITEMENT,ci.ID_ALERT_STATUT, ci.NOM FROM sign_incident_naturel s LEFT JOIN utilisateurs u on s.USER_ID=u.ID_UTILISATEUR LEFT JOIN statut_validation sv on sv.ID_STATUT=s.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts ci ON s.ID_ALERT_STATUT = ci.ID_ALERT_STATUT WHERE 1' . $condition1.' '.$critaire_date;
		...
		$search = !empty($_POST['search']['value']) ? ("AND NOM_UTILISATEUR LIKE '%$var_search%' OR s.DESCRIPTION LIKE '%$var_search%' ") : '';
		...
		foreach ($fetch_psr as $row) {
			...
			$option = "<a class='btn btn-primary' href='javascript:;'' onclick='getDetailControl(".$row->ID_SIGN_INCIDENT.")'' >Traiter</i></a>";
			 if ($row->ID_ALERT_STATUT==4) {
					$option = "<a class='btn btn-primary' href='javascript:;'' onclick='fin_trait(".$row->ID_SIGN_INCIDENT.")'' >Marqué comme terminer</i></a>";
				} elseif($row->ID_ALERT_STATUT==6){
					$option = "Le traitement est terminé";
				}
				if($row->ID_ALERT_STATUT==5){

					$option = "Le signalement est rejetté";
    ...
    //   $detail= $this->Model->getOne('sign_incident_naturel', array('ID_SIGN_INCIDENT'=>$id));
      

    //   // $data= $this->Model->getRequete('SELECT ID_UTILISATEUR,NOM_CITOYEN,PRENOM_CITOYEN FROM utilisateurs ORDER by NOM_CITOYEN');

    //   // $data['stat'] = $this->Model->getRequete('SELECT ID_STATUT,DESCRIPTION FROM statut_validation ORDER by DESCRIPTION');
    ...
    function detail($id){
		...
		$data['alerte']=$this->Model->getRequeteOne('SELECT ID_SIGN_INCIDENT, s.DESCRIPTION, s.IMAGE_1, s.IMAGE_2, s.IMAGE_3, s.VALIDATION, s.LONGITUDE, s.LATITUDE, s.DATE_INSERTION as date, u.NOM_UTILISATEUR,USER_ID,concat(u.NOM_CITOYEN," ",u.PRENOM_CITOYEN)as NAME,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,s.STATUT_TRAITEMENT, ci.NOM FROM sign_incident_naturel s LEFT JOIN utilisateurs u on s.USER_ID=u.ID_UTILISATEUR LEFT JOIN statut_validation sv on sv.ID_STATUT=s.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts ci ON s.ID_ALERT_STATUT = ci.ID_ALERT_STATUT WHERE ID_SIGN_INCIDENT ='.$id.'');

		$titre = "Detail d'une alerte";

		$map = $this->load->view('sign_incident_det_view', $data, TRUE);
	...
	function traiter_conf($id){
         $data = $this->Model->update('sign_incident_naturel',array('ID_SIGN_INCIDENT'=>$id),array('ID_ALERT_STATUT'=>4));
            if($data){
            $output = array('status'=> true,'id'=>$id);	
            }
	...
	function traiter_rejet($id){
         $data = $this->Model->update('sign_incident_naturel',array('ID_SIGN_INCIDENT'=>$id),array('ID_ALERT_STATUT'=>5));
            if($data){
            $output = array('status'=> true,'id'=>$id);	
            }
	...
	function fin_valide($id){
		$data = $this->Model->update('sign_incident_naturel',array('ID_SIGN_INCIDENT'=>$id),array('ID_ALERT_STATUT'=>6));
        if($data){
            $output = array('status'=> true,'id'=>$id);	
        }
