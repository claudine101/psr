...
class Sign_autres_signal extends CI_Controller
{
	...
	function index()
	{
		$autre = $this->Modele->getRequete('SELECT date_format(DATE_INSERTION,"%d-%m-%Y") as date FROM sign_autres_signalement WHERE 1 GROUP BY date_format(DATE_INSERTION,"%d-%m-%Y")');
		$data['autre'] = $autre;
	...
	function listing()
	{
       ...
       $critaire_date = " ";
        ...
        $order_column = array('NOM_UTILISATEUR', 's.DESCRIPTION', 's.DATE_INSERTION');
		...
		$search = !empty($_POST['search']['value']) ? ("AND NOM_UTILISATEUR LIKE'%$var_search%' OR s.DESCRIPTION LIKE '%$var_search%' OR s.DATE_INSERTION LIKE '%$var_search' ") : '';
		...
		foreach ($fetch_psr as $row) {
		    ...
		    $option = "<a class='btn btn-primary' href='javascript:;'' onclick='getDetailControl(".$row->ID_AUTRES_SIGNALEMENT.")'' >Traiter</i></a>";
		    if ($row->ID_ALERT_STATUT==4) {
					$option = "<a class='btn btn-primary' href='javascript:;'' onclick='fin_trait(".$row->ID_AUTRES_SIGNALEMENT.")'' >Marqué comme terminer</i></a>";
	...
	function detail($id){
		...
		$data['alerte']=$this->Model->getRequeteOne('SELECT ID_AUTRES_SIGNALEMENT, s.DESCRIPTION, IMAGE_1, IMAGE_2, IMAGE_3, VALIDATION, s.LONGITUDE, s.LATITUDE, s.DATE_INSERTION as date, u.NOM_UTILISATEUR,USER_ID,concat(u.NOM_CITOYEN," ",u.PRENOM_CITOYEN)as NAME,sv.ID_STATUT as status,sv.DESCRIPTION as STATUT_TRAITEM,s.STATUT_TRAITEMENT,ci.NOM FROM sign_autres_signalement s LEFT JOIN utilisateurs u on s.USER_ID=u.ID_UTILISATEUR LEFT JOIN statut_validation sv on sv.ID_STATUT=s.STATUT_TRAITEMENT LEFT JOIN civil_alerts_statuts ci on s.ID_ALERT_STATUT=ci.ID_ALERT_STATUT WHERE ID_AUTRES_SIGNALEMENT ='.$id.'');

		$titre = "Detail d'une alerte";

		$map = $this->load->view('sign_autre_det_v', $data, TRUE);
	...
	function traiter_conf($id){
         $data = $this->Model->update('sign_autres_signalement',array('ID_AUTRES_SIGNALEMENT'=>$id),array('ID_ALERT_STATUT'=>4));
            if($data){
	...
	function traiter_rejet($id){
         $data = $this->Model->update('sign_autres_signalement',array('ID_AUTRES_SIGNALEMENT'=>$id),array('ID_ALERT_STATUT'=>5));
            if($data){
            $output = array('status'=> true,'id'=>$id);	
            }
	...
	function fin_valide($id){
		$data = $this->Model->update('sign_autres_signalement',array('ID_AUTRES_SIGNALEMENT'=>$id),array('ID_ALERT_STATUT'=>6));
        if($data){
            $output = array('status'=> true,'id'=>$id);	
        }
