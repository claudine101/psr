	...
	function change_pswd()
	{
		...
		if ($this->form_validation->run() == FALSE) {
	  	...
	  	$data['title'] = 'Modification du mot de passe';
			$this->load->view('Change_Password_View', $data);
				...
				$criteres['USER_EMAIL']=$this->session->userdata('USER_NAME');
			$Old_Password=$this->Modele->getOne('admin_users',$criteres);

			if ($Old_Password['USER_PASSWORD'] == md5($this->input->post('Old_Password'))) {
				# code...
			  ...
			  $this->Modele->update('admin_users',$criteres,$data);
        ...
        redirect(base_url('Login/do_logout'));
           ...
           /*redirect(base_url('Change_Password/index'));*/
				...
				$data['message1']='<div class="alert alert-danger text-center" id="message">Les deux mots de passe ne sont pas identiques</div>';
        ...
        redirect(base_url('Change_Password/index'));
				...
				$data['message1']='<div class="alert alert-danger text-center" id="message">L\'ancien mot de passe est incorrect</div>';
